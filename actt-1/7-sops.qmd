---
title: "ACTT-1: State occupancy probabilities"
author: "Max Rohde"
date: today
toc: true
toc-depth: 4
...
write_SOP <- function(i, tx, baseline_df){
  ...
  soprobMarkovOrdm(
    mod,
    data = list(tx = tx, age = row$age, sex=row$sex, yprev=row$yprev),
    times = 1:28,
    ylevels = 1:8,
    absorb = 8,
    tvarname = "day",
...
# Placebo
if (generate_SOPS) {
    furrr::future_walk(1:nrow(baseline_df), \(x) write_SOP(x, "Placebo", baseline_df),
                       .progress=TRUE,
...
# Remdesivir
if (generate_SOPS) {
    furrr::future_walk(1:nrow(baseline_df), \(x) write_SOP(x, "Remdesivir", baseline_df),
                       .progress=TRUE,
                       .options = furrr_options(seed=TRUE))
}
```
...
files <- fs::dir_ls("./marginalized_sop_placebo/", glob = "*.parquet")
...
```{r}
# Get list of all files
files <- fs::dir_ls("./marginalized_sop_remdesivir/", glob = "*.parquet")
...
```{r}
sop_df <- bind_rows(placebo_df, tx_df)
```
